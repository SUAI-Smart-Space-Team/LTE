...
server.on('error', (err) => { // error output
  console.log(`server error:\n${err.stack}`);
  server.close();
});

server.on('message', (msg, rinfo) => { //the server accepts the request and performs the action in this part
  console.log(`server got: ${msg} from ${rinfo.address}:${rinfo.port}`);
  var Ansible = require('node-ansible');
  var data = [];
  data = JSON.parse(msg);
  if(data.action == 'add_rule') { //add a rule to the masquerade
  ...
  if(data.action == 'delete_rule') { //delete a rule from a masquerade
         console.log(data.action);
         console.log(data.ip_in);
         console.log(data.ip_out);
  ...
  if(data.action == 'delete_all_rule') { //delete all rules from a masquerade
         console.log(data.action);
  ...
  if(data.action == 'get_ip') { //get an external ip
         console.log(data.action);
         console.log(data.ip_in);
  ...
  if(data.action == 'modify_ip') { //modify ip in masquerade
         console.log(data.action);
  ...
  if(data.action == 'modify_rule') { //modify rule in masquerade
         console.log(data.action);
  ...
  if(data.action == 'ping_show_iptables') { //show iptables
         console.log(data.action);
