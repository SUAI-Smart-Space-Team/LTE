...
public class Server {
    ...
    private Connection connectionToSite;
    ...
    protected void acceptServer() {
        while (true) {
            try {
                ...
                new ServerThread(socket,connectionToSite).start();
            } catch (Exception e) {
    ...
    //метод подключения клиента к сайту
    protected void connectToSite(String serverAddress,int port) {
        //если клиент не подключен  сервере то..
        if (!isConnect) {
            while (true) {
                try {
                    //вызываем окна ввода адреса, порта сервера
                    serverAddress = gui.getSiteServerAddressFromOptionPane();
                    port = gui.getSitePortFromOptionPane();
                    //создаем сокет и объект connection
                    Socket socket = new Socket(serverAddress, port);
                    connectionToSite = new Connection(socket);
                    isConnect = true;
                    gui.refreshDialogWindowServer("Подключился к сайту с данными.\n");
                    break;
                } catch (Exception e) {
                    gui.refreshDialogWindowServer("Произошла ошибка при подключении к серверу сайта. Проверьте ip и т.д.");
                    break;
                }
            }
        }
    ...
    private class ServerThread extends Thread {
        ...
        public ServerThread(Socket socket, Connection connectionToSite) {
            this.socket = socket;
        ...
        //метод, реализующий обмен json файлами между приложением и сайтом
        private void messagingWithSite(Connection connection) {
            while (true) {
                try {
                    //класс для работы с json
                    ...
                    gui.refreshDialogWindowServer(String.format("Произошла ошибка при получении json файла\n"));
                    break;
                }
            }
        }
        ...
        public void run() {
            ...
            try {
                ...
                messagingWithSite(connectionToSite);
